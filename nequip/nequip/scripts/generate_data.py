...
def fresh_start(config):
    ...
    atom_id = torch.arange(432) + 1
    atom_type = torch.concat((torch.ones(324), torch.ones(108) * 2))
    for i in range(10):
        atom_pos = dataset[i].pos.type(torch.float16)
        with open(f"nh3_{i}.data", 'w') as f:
            f.write("# LAMMPS data file written by OVITO Pro 3.7.10\n")
            f.write("432 atoms\n")
            f.write("2 atom types\n")
            f.write("0.0 16.51501 xlo xhi\n")
            f.write("0.0 16.51501 ylo yhi\n")
            f.write("0.0 16.51501 zlo zhi\n")
            f.write("\n")
            f.write("Masses\n")
            f.write("\n")
            f.write("1 2.014012  #1.00794  # H\n")
            f.write("2 14.0067  # N\n")
            f.write("\n")
            f.write("Atoms  # atomic\n")
            f.write("\n")
            for j in range(432):
                f.write(f"{int(atom_id[j])} {int(atom_type[j])} {atom_pos[j][0]} {atom_pos[j][1]} {atom_pos[j][2]}\n")
    exit()
    # = Train/test split =

...
def main(args=None, running_as_script: bool = True):
    ...
    with contextlib.ExitStack() as context_stack:
        ...
        while True:
            ...
            original_loss.append(loss(out, batch)[0])
            with torch.no_grad():
                # Write output
                display_bar.set_description_str(
                    f"Train Loss: {original_loss[-1]:4.4f}"
                )
            if batch_i == 5:
                break
            batch_i += 1
    ...
    with torch.no_grad():
        for p, original_p in zip(copied_model.parameters(), model.parameters()):
            p.copy_(original_p + torch.randn_like(original_p) * 0.01)
    batch_i: int = 0
    batch_size: int = args.batch_size
    ...
    with contextlib.ExitStack() as context_stack:
        ...
        while True:
            ...
            with torch.no_grad():
                # Write output
                display_bar.set_description_str(
                    f"Perturbed Train Loss: {perturbed_loss[-1]:4.4f}"
                )
            if batch_i == 5:
                break
            batch_i += 1
        ...
        prog.close()
    print(sum(perturbed_loss) / len(perturbed_loss) - sum(original_loss) / len(original_loss))
    exit()

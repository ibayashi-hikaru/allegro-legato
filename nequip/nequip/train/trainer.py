...
class Trainer:
    ...
    def init_objects(self):
        ...
        if self.with_sam:
            _, self.optimizer_kwargs = instantiate_from_cls_name(
                module=torch.optim,
            ...
            class_list = inspect.getmembers(torch.optim, inspect.isclass)
            class_dict = {}
            for k, v in class_list:
                class_dict[k] = v
            base_optim = class_dict.get(self.optimizer_name, None)
            self.optim = SAM(self.model.parameters(), base_optim, lr=self.learning_rate,
                             rho=self.rho, adaptive=self.adaptive_sam)
        else:

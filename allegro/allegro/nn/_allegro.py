...
class Allegro_Module(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        self._input_pad = (
            int(math.ceil(env_embed_irreps.dim / pad_to_alignment)) *
            pad_to_alignment
        ...
        for layer_idx, (arg_irreps, out_irreps) in enumerate(
            ...
            assert all(ir == SCALAR for _,
                       ir in full_out_irreps[:n_scalar_outs])
            tp = Contracter(
                    ...
                    two_body_latent(
                        mlp_input_dimension=(
                            (
                                ...
                                2 * \
                                self.irreps_in[self.node_invariant_field].num_irreps
        ...
        self.register_buffer("per_layer_cutoffs",
                             torch.as_tensor(per_layer_cutoffs))
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        for latent, env_embed_mlp, env_linear, tp, linear in zip(
            ...
            new_latents = cutoff_coeffs[active_edges].unsqueeze(
                -1) * new_latents
            ...
            if self.latent_resnet and layer_index > 0:
                ...
                coefficient_old = torch.rsqrt(
                    this_layer_update_coeff.square() + 1)
            ...
            if self.embed_initial_edge and layer_index == 0:
                # embed initial edge
                env_w = weights.narrow(-1, w_index,
                                       self._env_weighter.weight_numel)

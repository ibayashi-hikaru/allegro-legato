...
bool essential_logging_started = false;
void utils::logmesg(LAMMPS *lmp, const std::string &mesg)
{
  ...
  if (essential_logging_started) {
    if (std::stoi(tokens[1]) < 100 || std::stoi(tokens[1]) > 300) { MPI_Abort(MPI_COMM_WORLD, 1); }
  } else {
    if (tokens[0] == "Step") { essential_logging_started = true; }
  }
}
